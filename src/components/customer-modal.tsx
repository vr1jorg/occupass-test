...
export default function CustomerModal() {
    ...
    return (
        selectedCustomerId ? <div className="fixed flex flex-col w-full h-full inset-0 z-50 bg-slate-100">
            <button className="ml-auto p-4 absolute right-2 top-2 cursor-pointer" onClick={() => selectCustomer(null)}><CloseIcon /></button>
            <CustomerInformation />
            <CustomerOrders />
        </div > : null
    )
